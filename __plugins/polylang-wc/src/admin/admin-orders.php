...
abstract class PLLWC_Admin_Orders {
	...
	/**
	 * Updates the download links with the order language.
	 *
	 * @since 2.2
	 *
	...
	public function update_order_download_links( $order_object ) {
		...
		$language = PLL()->model->get_language( $this->data_store->get_language( $order->get_id() ) );
		...
		while ( $content_tags->next_tag( array( 'tag_name' => 'a', 'class_name' => 'button' ) ) ) {
			...
			$content_tags->set_attribute( 'href', esc_url( PLL()->links_model->switch_language_in_link( $content_tags->get_attribute( 'href' ), $language ) ) );
		}

		echo $content_tags->get_updated_html(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
	...
	protected function is_allowed_screen( $screen_id = '' ) {
		...
		return in_array( $screen->id, $this->get_allowed_screens(), true );
	}
