...
class PLLWC_Wizard
{
    public const PLUGIN_FILTER = ['woocommerce', 'polylang', 'polylang-pro', 'polylang-wc'];
    ...
    public function __construct($model, $wizard)
    {
        ...
        add_filter('pll_wizard_steps', [$this, 'add_step_wc_pages'], 600);
    }
    ...
    public function add_step_wc_pages($steps)
    {
        ...
        $steps['wc-pages'] = [
            'name'    => 'WooCommerce',
            'view'    => [$this, 'display_step_wc_pages'],
            'handler' => [$this, 'save_step_wc_pages'],
            'scripts' => [],
            'styles'  => ['woocommerce_admin_styles'],
        ];
    ...
    {
        add_filter('site_transient_update_plugins', [$this, 'update_last_checked']);
        wp_update_plugins(); // To ensure the transient is updated recently.
        remove_filter('site_transient_update_plugins', [$this, 'update_last_checked']);
    ...
    /**
     * Filters the `update_plugins` transient last checked date.
     *
     * @since 1.4
     *
    ...
    {
        // Unset to always search for the translations updates. See `wp_update_plugins()`.
        unset($updates->last_checked);
