...
class PLLWC_Product_Language_CPT extends PLLWC_Translated_Object_Language
{
    ...
    public function updated_props($product, $updated_props)
    {
        ...
        foreach ($this->get_translations($id) as $tr_id) {
            if ($id && $id !== $tr_id && $tr_product = wc_get_product($tr_id)) {
                ...
                if (array_intersect($updated_props, $this->get_meta_lookup_props())) {
    ...
    /**
     * Checks if WooCommerce COGS feature is enabled.
     *
     * @since 2.2
     *
     * @return bool true if COGS is enabled, false otherwise.
     */
    ...
    /**
     * Checks if the COGS value column from the product meta lookup table can be used.
     * This column could be missing because a user action is required to add it.
     *
     * @since 2.2
     *
     * @return bool true if the COGS value column from the product meta lookup table can be used, false otherwise.
     */
    ...
    /**
     * Gets properties stored in the table `wc_product_meta_lookup`.
     *
     * @since 2.2
     *
     * @return string[] An array of properties stored in the table `wc_product_meta_lookup`.
     */
