...
class PLLWC_Product_Import
{
    ...
    public function __construct()
    {
        ...
        add_filter('woocommerce_csv_product_import_mapping_default_columns', [$this, 'default_columns']);
        add_filter('woocommerce_csv_product_import_mapping_options', [$this, 'mapping_options'], 1);
        add_action('woocommerce_product_import_inserted_product_object', [$this, 'inserted_product_object'], 10, 2);

        add_action('woocommerce_product_importer_before_set_parsed_data', [$this, 'before_set_parsed_data'], 10, 2);
        add_action('woocommerce_product_import_before_import', [$this, 'set_language']);
        add_action('woocommerce_product_import_before_process_item', [$this, 'set_language']);

        add_filter('woocommerce_product_importer_formatting_callbacks', [$this, 'formatting_callbacks'], 10, 2);
    }
    ...
    public function set_translation_group($id, $data)
    {
        ...
        if (empty($term)) {
            ...
            $term = wp_insert_term($group, $taxonomy, ['description' => serialize($translations)]); // phpcs:ignore WordPress.PHP.DiscouragedPHPFunctions
            if (!is_wp_error($term)) {
    ...
    public function before_set_parsed_data($row, $mapped_keys)
    {
        ...
        add_filter('woocommerce_get_product_id_by_sku', [$this, 'get_product_id_by_sku'], 10, 2);
        add_filter('pllwc_language_for_unique_sku', [$this, 'language_for_unique_sku']);
    ...
    public function formatting_callbacks($callbacks, $importer)
    {
        ...
        if (false !== $key = array_search('category_ids', $importer->get_mapped_keys())) {
            $callbacks[$key] = [$this, 'parse_categories_field'];
        }

        if (false !== $key = array_search('tag_ids', $importer->get_mapped_keys())) {
            $callbacks[$key] = [$this, 'parse_tags_field'];
        }
