...
class PLLWC_Product_Bundles
{
    ...
    public function __construct()
    {
        ...
        add_action('pllwc_copy_product', [$this, 'copy_product'], 10, 4);
        add_filter('pllwc_copy_post_metas', [$this, 'copy_product_metas']);
        add_filter('pll_post_metas_to_export', [$this, 'get_metas_to_translate']);
        ...
        foreach ($from->get_bundled_data_items() as $item) {
            ...
            if (!empty($meta_data['default_variation_attributes'])) {
                ...
                foreach ($meta_data['default_variation_attributes'] as $k => $v) {
                    ...
                    $terms = get_terms(['taxonomy' => $k, 'slug' => $v, 'lang' => '']); // Don't use get_term_by filtered by language since WP 4.7.
                    if (!is_array($terms)) {
                        continue;
                    }
            ...
            if ($sync) {
                $meta_data = array_diff_key($meta_data, array_flip(['title', 'description'])); // Copy, don't sync.
            }
    ...
    public function copy_product_metas($metas)
    {
        $to_sync = [
            ...
            '_wc_pb_bundle_sells_discount',
            '_wc_pb_bundle_sell_ids',
        ];
    ...
    /**
     * Adds the product metas to translate.
     *
     * @since 2.2
     *
     * @param array $metas An array of product metas to translate.
     *
     * @return array
     */
    public function get_metas_to_translate($metas)
    {
        return array_merge($metas, ['_wc_pb_bundle_sells_title' => 1]);
    }

    /**
     * Translates the Bundle-sells IDs of a product.
    ...
    public function translate_post_meta($value, $key, $lang)
    {
        ...
        foreach ($value as $product_id) {
            $tr_id = $data_store->get($product_id, $lang);
            if (empty($tr_id)) {
                continue;
            }

            $bundle_sells_tr_ids[] = $tr_id;
        }

        return $bundle_sells_tr_ids;
    }
    ...
    protected function translate_stamp($item, $lang)
    {
        ...
        foreach ($item['stamp'] as $s) {
            $orig_lang = PLL()->model->get_language((string) $data_store->get_language($s['product_id']));
            if (!$orig_lang) {
                continue;
            }
            ...
            if ($tr_id && !empty($bundled_items[$tr_id])) {
                ...
                if (!empty($s['attributes'])) {
                    $s['attributes'] = (new Item($s))->translate_attributes($s['attributes'], $lang, $orig_lang);
                }
