...
class PLLWC_Links
{
    ...
    {
        // Rewrite rules.
        add_action('pll_prepare_rewrite_rules', [$this, 'prepare_rewrite_rules'], 5); // Before Polylang.
        add_filter('pll_modify_rewrite_rule', [$this, 'fix_rewrite_rules'], 10, 4);
        ...
        if (PLL() instanceof PLL_Frontend) {
            add_filter('option_woocommerce_permalinks', [$this, 'option_woocommerce_permalinks']);
        }

        add_filter('woocommerce_get_checkout_order_received_url', [$this, 'checkout_order_received_url'], 10, 2);

        if (PLL()->options['force_lang'] > 1) {
            add_filter('woocommerce_customer_available_downloads', [$this, 'filter_download_urls']);
            add_filter('woocommerce_get_item_downloads', [$this, 'filter_download_urls']);
            add_filter('woocommerce_product_file_download_path', [$this, 'filter_download_url']);
        }
    }
     ...
     * @since 2.2 Removed param and return value, the method being hooked to an action instead of a filter.
     *
     * @return void
     */
    public function prepare_rewrite_rules()
    {
        if (!has_filter('rewrite_rules_array', [$this, 'rewrite_shop_rules'])) {
            add_filter('rewrite_rules_array', [$this, 'rewrite_shop_rules'], 5); // Before Polylang.
            add_filter('rewrite_rules_array', [$this, 'rewrite_shop_subpages_rules'], 20); // After wc_fix_rewrite_rules().
        }
    }
            ...
            if ($tr_shop_id) {
                ...
                foreach (wc_get_attribute_taxonomies() as $tax) {
                    ...
                    if (!empty($_GET[$name]) && $tr_id = pll_get_term((int) $_GET[$name], $lang)) { // phpcs:ignore WordPress.Security.NonceVerification
                        $url = add_query_arg([$name => $tr_id], $url);
                    }
                }
            }
        }
    ...
    /**
     * Filters downloads URL to switch them to the current language.
     *
     * @since 2.2
     *
     * @param array $downloads List of downloads.
     *
     * @return array List of downloads updated with their URL corresponding to the current language.
     */
    public function filter_download_urls($downloads)
    {
        if (!is_array($downloads) || !PLL()->curlang instanceof PLL_Language) {
            return $downloads;
        }
        ...
        foreach ($downloads as $key => $download) {
            ...
            $downloads[$key]['download_url'] = PLL()->links_model->switch_language_in_link($download['download_url'], PLL()->curlang);
        }
    ...
    /**
     * Filters one download URL to switch it to the current language.
     * This prevents to trigger a "File not found" error when trying to download a file.
     *
     * @since 2.2
     * @see WC_Download_Handler::download_product()
     *
     * @param string $download_url File download URL.
     *
     * @return string
     */
    public function filter_download_url($download_url)
    {
