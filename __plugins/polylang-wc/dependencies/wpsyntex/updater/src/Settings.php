...
class Settings extends PLL_Settings_Module
{
    ...
    public function __construct(PLL_Settings &$polylang)
    {
         ...
         */
        remove_action('wp_ajax_pll_save_options', [$this, 'save_options']);
         ...
         */
        add_action('wp_ajax_pll_save_options', [$this, 'save_options'], 5);

        add_action('wp_ajax_'.self::GET_LICENSES_DATA_ACTION, [$this, 'display_licenses']);
    }
        ...
        foreach ($this->get_licenses() as $item) {
            ...
            $response->Add(
                [
                    ...
                    'data'         => $item->id,
                    'supplemental' => ['html' => $updated_item->get_form_field()],
                ]
            );
