...
class PLL_TEC
{
     ...
     * @var string[]
     */
    protected $translatable_slug_ids = [];
    ...
    public function init($polylang)
    {
        ...
        $this->is_tec_rest_request = [];
        $this->translatable_slug_ids = [];
        ...
        add_action('save_post_'.Venue::POSTTYPE, [$this, 'set_language'], 10, 3);
        add_action('save_post_'.Organizer::POSTTYPE, [$this, 'set_language'], 10, 3);

        $tec = TEC::instance();

        if (empty($polylang->options['force_lang'])) {
            add_action('pll_language_defined', [$this, 'fix_date_translations']);
        }

        self::$metas = array_merge($tec->metaTags, $tec->venueTags, $tec->organizerTags, ['_VenueShowMap', '_VenueShowMapLink']);
        ...
        if (!empty($this->links) && !empty($GLOBALS['post'])) {
            ...
            if (!empty($data)) {
                ...
                foreach (self::$metas as $meta) {
                    ...
                    add_filter('tribe_get_meta_default_value_'.$filter, [$this, 'copy_event_meta'], 10, 4); // Since TEC 4.0.7.
                }
        ...
        add_action('pre_get_posts', [$this, 'pre_get_posts'], 60); // After `Tribe__Events__Query->pre_get_posts()`.

        add_filter('pll_copy_post_metas', [$this, 'copy_post_metas']);
        add_filter('pll_translate_post_meta', [$this, 'translate_meta'], 10, 3);

        // Translate links with translated slugs
        add_action('init', [$this, 'reset_slugs'], 11); // Just after `Tribe__Events__Main->init()`.
        add_filter('register_taxonomy_args', [$this, 'register_taxonomy_args'], 10, 2);
        add_filter('tribe_events_get_link', [$this, 'get_link']);
        add_filter('pll_get_archive_url', [$this, 'pll_get_archive_url'], 10, 2);
        add_filter('pll_term_link', [$this, 'filter_tec_term_link'], 5, 3); // Before `PLL_Translate_Slugs->pll_term_link()`.
        add_filter('pll_translated_slugs', [$this, 'pll_translated_slugs'], 10, 3);
        add_filter('pll_sanitize_string_translation', [$this, 'sanitize_string_translation'], 10, 2);
        add_filter('tribe_events_rewrite_i18n_slugs_raw', [$this, 'rewrite_slugs']);
        ...
        $args = [
            ...
            'sanitize_callback' => [$this, 'sanitize_strings'],
        ];
        ...
        add_filter('tribe_events_rewrite_i18n_slugs', [$this, 'fix_escaped_dashes_in_slugs'], 10, 2);

        add_filter('tribe_events_category_slug', [$this, 'get_category_slug']);
        add_filter('tribe_events_tag_slug', [$this, 'get_tag_slug']);

        add_filter('tribe_events_views_v2_endpoint_url', [$this, 'add_missing_lang_to_rest_url']);
        add_filter('locale', [$this, 'filter_locale_for_rest'], 5);

        add_filter('tribe_events_views_v2_publicly_visible_views_query_args', [$this, 'add_language_to_publicly_visible_views_query_args'], 5);
        add_filter('tribe_events_views_v2_view_template_vars', [$this, 'translate_widget_view_more_link'], 5);
        add_filter('tribe_rewrite_parse_query_vars', [$this, 'force_language_on_tec_parse_query_vars'], 10, 3);
        add_filter('tribe_events_views_v2_url_query_args', [$this, 'add_missing_lang_to_query_arg']);
        add_filter('tribe_rewrite_pre_canonical_url', [$this, 'add_missing_lang_to_non_canonical_url'], 10, 2);
        add_filter('tribe_rewrite_pre_canonical_url', [$this, 'remove_name_arg_from_non_canonical_url'], 10, 2);

        add_filter('tribe_rewrite_canonical_url', [$this, 'fix_language_in_canonical_url'], 5, 2);
        add_filter('tribe_rewrite_canonical_url', [$this, 'translate_canonical_url'], 5, 2);
    }
    ...
    public function translate_taxonomies($taxonomies, $hide)
    {
        ...
        return $hide ? array_diff($taxonomies, [TEC::TAXONOMY]) : array_merge($taxonomies, [TEC::TAXONOMY]);
    }
    ...
    {
        $tec_types = [TEC::POSTTYPE, TEC::VENUE_POST_TYPE, TEC::ORGANIZER_POST_TYPE];
    ...
    public function pre_get_posts()
    {
        if (wp_doing_ajax() && isset($_GET['action']) && 'pll_posts_not_translated' === $_GET['action']) { // phpcs:ignore WordPress.Security.NonceVerification
            ...
            remove_filter('posts_where', ['Tribe__Events__Query', 'posts_where']);
            remove_filter('posts_fields', ['Tribe__Events__Query', 'posts_fields']);
            remove_filter('posts_orderby', ['Tribe__Events__Query', 'posts_orderby']);
        }
    }
        ...
        foreach ($bases as $type => $base) {
            ...
            foreach ($translatable_slugs as $slugs) {
                if ($slugs['slug'] === $default_slug) {
                    $bases[$type] = array_unique(array_merge([$default_slug], $slugs['translations']));
                    break;
                }
            }
        }
    ...
    public function add_missing_lang_to_rest_url($url)
    {
        ...
        return add_query_arg(['lang' => $curlang->slug], $url);
    }
